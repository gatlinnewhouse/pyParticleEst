...
def generate_dataset(length, Qz, R, Qes, Qeb):
    ...
    for i in range(1, length + 1):
        ...
        wz = (
            numpy.random.multivariate_normal(numpy.zeros((1,)), Qz)
            .ravel()
            .reshape((-1, 1))
        )
        ...
        a = 1 if (t % 2 == 0) else 0
        y[:, i - 1] = (
            e**2 + (1 - a) * z + numpy.random.multivariate_normal(numpy.zeros((1,)), R)
        ).ravel()
        e_vec[:, i] = e.ravel()
        z_vec[:, i] = z.ravel()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "apf_compare":
            ...
            for ind, pc in enumerate(part_count):
                divind = (
                    numpy.isnan(rmse_eta[:, ind])
                    | numpy.isinf(rmse_eta[:, ind])
                    | numpy.isnan(rmse_theta[:, ind])
                    | numpy.isinf(rmse_theta[:, ind])
