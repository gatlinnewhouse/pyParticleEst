...
def generate_dataset(steps, P0_xi, P0_z, Q_xi, Q_z, R_xi, R_z):
    ...
    z = np.zeros((2, steps + 1))
    y = np.zeros((steps, 2))
    xi[:, 0] = np.random.normal(0.0, math.sqrt(P0_xi))
    z[:, 0] = np.random.multivariate_normal(np.zeros((2,)), P0_z)
    for k in range(1, steps + 1):
        xi[:, k] = xi[:, k - 1] + np.random.normal(0.0, Q_xi)
        Ak = np.asarray(
            (
        ...
        y[k - 1, 1] = C.dot(z[:, k]) + np.random.normal(0.0, math.sqrt(R_z))
...
class Model(HierarchicalRSBase):
    ...
    def __init__(self, P0_xi, P0_z, Q_xi, Q_z, R_xi, R_z) -> None:
        ...
        self.Q_xi = np.copy(Q_xi)
        self.R_xi = np.copy(R_xi)
        self.P0_z = np.copy(P0_z)
        self.R_z = np.copy(R_z)
        fz = np.zeros((2, 1))
        hz = np.zeros((1, 1))
        self.pn_count = 0
    ...
    def create_initial_estimate(self, N):
        particles = np.zeros((N, self.lxi + self.kf.lz + 2 * self.kf.lz**2))

        for i in range(N):
            particles[i, 0] = np.random.normal(0.0, math.sqrt(self.P0_xi))
            particles[i, 1:3] = np.zeros((1, 2))
            particles[i, 3:7] = np.copy(self.P0_z).ravel()
        return particles

    def get_rb_initial(self, xi0):
        N = len(xi0)
        z0 = np.zeros((self.kf.lz, 1))
        z_list = np.repeat(z0.reshape((1, self.kf.lz, 1)), N, 0)
        P_list = np.repeat(self.P0_z.reshape((1, self.kf.lz, self.kf.lz)), N, 0)
        return (z_list, P_list)

    def sample_process_noise(self, particles, u, t):
        """Return process noise for input u"""
        N = len(particles)
        return np.random.normal(0.0, math.sqrt(self.Q_xi), (N,))
